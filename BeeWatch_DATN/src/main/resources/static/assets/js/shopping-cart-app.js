...
app.controller("shopping-cart-ctrl", function ($scope, $http, $window) {
    ...
    $scope.cart = {
        ...
        add(productId) {
            ...
            if (item) {
                ...
                $http.get(`/rest/products/${productId}`).then((resp) => {
                    resp.data.qty = 1;
                    if (
                        resp.data.category.discount != null &&
                        resp.data.category.discount != ""
                    ) {
                        resp.data.price =
                            resp.data.price -
                            (resp.data.price * resp.data.category.discount.percentDiscount) /
                            100;
                    }
                    this.items.push(resp.data);
                    this.saveToLocalStorage();
        ...
        checkQuantity(index) {
            ...
            if (this.items[index].quantity <= this.items[index].qty) {
                ...
                var cf = confirm(
                    "Bạn muốn xóa " + this.items[index].name + " ra khỏi giỏ hàng?"
                );
                if (cf == true) {
                    this.remove(index);
                } else {
    ...
    $scope.totalFinalOLogin = $scope.cart.amount;
    //Tên voucher
    $scope.result = "";
    //Giá trị voucher
    $scope.result2 = "0";
    var isAccount = document.getElementById("isAccount")
    $scope.voucherVal = {
        cates: [],
        //lấy % của voucher
        getValuedVoucher(result) {
            ...
            $http.get(`/rest/vouchers/${resultName}`).then((resp) => {
                ...
                console.log("isAccount: ", isAccount.value)
                if (isAccount.value == 1) {
                    resp.data.finalPrice = totalPriceLogin - resp.data.valued;
                    resp.data.priceStart = totalPriceLogin;
                } else {
                    resp.data.finalPrice = $scope.totalFinalOLogin - resp.data.valued;
                    resp.data.priceStart = $scope.totalFinalOLogin;
                }
                ...
                if (Number(totalPriceLogin) >= dieuKien || Number($scope.totalFinalOLogin) >= dieuKien) {
                    alert("Áp dụng thành công");
                    $scope.totalFinal = resp.data.priceStart - resp.data.valued;
                    $scope.voucherVal.infoVoucher();
                } else {
                    alert(
                        "Không dùng được (Đơn hàng phải lớn hơn hoặc bằng " + dieuKien + ")"
                    );
                    $scope.result = "";
                    $scope.result2 = "0";
